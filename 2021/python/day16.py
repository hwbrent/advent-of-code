...
# Next 3 bits  --> packet type ID
    # If type ID is 4, it's a literal
    # Else, it's an operator

# ***** Literal *****
# - Left padded with 0s until its length is a multiple of 4
# - Then broken into groups of 4 bits
# - Each is prefixed by 1 (except the last which is prefixed by 0)
#   - (So in total, each group is 5 bits long)

# ***** Operator *****
# - Contains one or more packets
# - the bit immediately after the packet header is the length type ID:
#   - If 0 -> next 15 bits are a number representing the total length in
#     bits of the sub-packets contained by this packet.
#   - If 1 -> next 11 bits are a number representing the number of sub-packets
#     immediately contained by this packet.
# - After the type bit and 15/11 bits, the packets begin
